...
const inst = md({
  ...
  .use(rl);
inst.renderer.rules._image = inst.renderer.rules.image;
inst.renderer.rules.image = function (tokens, idx, options, env, slf) {
  const srcAttr = tokens[idx].attrs.find(a => a[0] === 'src');
  srcAttr[1] = helpers.unmark(srcAttr[1]);
  if (srcAttr[1].endsWith('.gif')) {
    return `<video autoplay loop poster="${helpers.markedPath(
      `${srcAttr[1]}?image`
    )}"><source src="${helpers.markedPath(
      `${srcAttr[1]}?webm`
    )}" type="video/webm"/></video>`;
  } else {
    return inst.renderer.rules._image(tokens, idx, options, env, slf);
  }
};
...
async function buildOutput(input, resPath, cb) {
  ...
  if (
    ...
    if (resPath.includes('debug')) {
      console.log(
        `module.exports = ${helpers.subRequires(
    ...
    const markedPaths = await helpers.markPaths(structuredInput, resPath);
    cb(
      null,
      `export default ${helpers.subRequires(JSON.stringify(markedPaths))}`
    );
    return;
  }
}
