...
const REPO_CACHE_LOCATION = '/.repo-cache/';
...
function cacheRepo(uriString, repoUrl = undefined) {
  ...
  var dir_relative = path.join(
    REPO_CACHE_LOCATION.startsWith('/')
      ? REPO_CACHE_LOCATION.substring(1)
      : REPO_CACHE_LOCATION,
    d.repo_user
  );
  const cmd = `
...
function repoUriToCachePath(uriString, fallbackUrl = undefined) {
  ...
  return `${REPO_CACHE_LOCATION}${d.repo_user}/${d.repo_name}/${d.file_path}`;
}
...
module.exports = {
  ...
  markPaths: (obj, resPath) => {
    ...
    Object.entries(obj).forEach(([k, v]) => {
      ...
      } else if (fs.existsSync(path.join(resDir, v))) {
  ...
  subRequiresJson: markedString => {
    return markedString.replace(REQUIRE_REGEX, `require('$1').default`);
  },
  subRequiresString: markedString => {
    return markedString.replace(REQUIRE_REGEX, "${require('$1')}");
  },
};
