...
const datasets = importYamls(require.context('../data/datasets', false, /\.yaml$/));
const projects = importYamls(require.context('../data/projects', false, /\.yaml$/));
...
function importYamls(req) {
  return Object.assign({}, ...req.keys().map((k) => ({[k.replace(/.*\/(.*)\.yaml$/g, '$1')]: req(k).default})));
}
