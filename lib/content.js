  ...
  Object.values(collections).forEach((p) => {
      ...
      p.code = p.code.map((c) => {
        if (!(c in code)) {
          throw new ContentError(
              `Collection '${p.id}' contains code with ID '${c}' which doesn't exist!` +
              `(offending collection file: ${p.src})`
          );
        }
        return code[c];
      });
    ...
    if (p.datasets) {
      p.datasets = p.datasets.map((d) => {
        if (!(d in datasets)) {
          throw new ContentError(
              `Collection '${p.id}' contains dataset with ID '${d}' which doesn't exist!` +
              `(offending collection file: ${p.src})`
          );
        }
        return datasets[d];
