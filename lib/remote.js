...
function rebuildRequired(name_short, current_hash) {
  const cache_info = rc.loadCacheInfo();
  const repo_path = path.join(rc.cacheLocation(), name_short);
  const opts = {
    cwd: repo_path,
    encoding: 'utf8',
    stdio: 'ignore',
  ...
  if (!fs.existsSync(repo_path)) return true;
  if (!cache_info[name_short]) return true;
