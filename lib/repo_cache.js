...
module.exports = {
  ...
  subRequires: (markedString, rootUrl, aggressive = false) => {
    return markedString.replace(
      ...
      (match, p1, p2, p3, offset, src) => {
        ...
        } else if (!/\.(md|gif|png|jpg|jpeg|webp)$/.test(p2)) {
        ...
        const pre = !p1 || p1 === '\\"' ? `${p1}"+` : '';
        const post = !p3 || p3 === '\\"' ? `+"${p3}` : '';
        return `${pre}require('${module.exports.unmark(
          p2,
          rootUrl,
          aggressive
        )}')${post}`;
  ...
  unmark: (markedString, rootUrl, aggressive = false) => {
    // console.log(`${markedString} (${rootUrl}, ${aggressive}) ->`);
    const unmarked = markedString
      .replace(REQUIRE_START, '')
      .replace(REQUIRE_END, '');
    const out = isRepoResource(unmarked, aggressive)
      ? repoUriToCachePath(unmarked, rootUrl)
      : unmarked;
    // console.log(`\t${out}`);
    return out;
  },
